...
export default function RouteTable({ routes, sortCol, sortAsc, sortCb }: IProps) {
  ...
  const addHeadingCell = (col: string | null, desc: string, span: number,
    ...
    if (col) {
      ...
      th = <th key={key} className={thClasses.join(' ')} style={style} onClick={() => sortCb(col)} colSpan={span}>{desc}&nbsp;{icon}</th>
