...
class Main extends Component<RouteComponentProps> {
  ...
  render = () => {
    const { picMed, fullName, perms } = this.context.auth
    ...
    function addRoute(url: string, exact: boolean, Component: ComponentType) {
      routes.push(
        <Route key={url} path={url} component={Component} exact={exact}/>
      )
    }

    function addNavUrl(url: string, exact: boolean, desc: string, Component: ComponentType, atEnd: boolean = true) {
